...
function hf($b){if($b<=0)return'0 B';$u=['B','KB','MB','GB'];$p=floor(log($b,1024));return round($b/pow(1024,$p),2).' '.$u[$p];}
$ext     = strtolower(pathinfo($m['filename'],PATHINFO_EXTENSION));

$dlFiles = [];
if (!empty($m['filename'])) {
  $abs = __DIR__.'/uploads/'.$m['filename'];
  if(is_file($abs)) $dlFiles[] = ['format'=>'original', 'ext'=>strtoupper(pathinfo($m['filename'], PATHINFO_EXTENSION)), 'size'=>hf(filesize($abs)), 'label'=>'Original format'];
}
if (!empty($m['file_gltf'])) {
  $abs = __DIR__.'/uploads/'.$m['file_gltf'];
  if(is_file($abs)) $dlFiles[] = ['format'=>'gltf', 'ext'=>'GLTF', 'size'=>hf(filesize($abs)), 'label'=>'Converted format'];
}
if (!empty($m['file_glb'])) {
  $abs = __DIR__.'/uploads/'.$m['file_glb'];
  if(is_file($abs)) $dlFiles[] = ['format'=>'glb', 'ext'=>'GLB', 'size'=>hf(filesize($abs)), 'label'=>'Converted format'];
}
if (!empty($m['file_usdz'])) {
  $abs = __DIR__.'/uploads/'.$m['file_usdz'];
  if(is_file($abs)) $dlFiles[] = ['format'=>'usdz', 'ext'=>'USDZ', 'size'=>hf(filesize($abs)), 'label'=>'Converted format'];
}
if (!empty($m['file_obj'])) {
  $abs = __DIR__.'/uploads/'.$m['file_obj'];
  if(is_file($abs)) $dlFiles[] = ['format'=>'obj', 'ext'=>'OBJ', 'size'=>hf(filesize($abs)), 'label'=>'Converted format'];
}
          ...
          <a href="model.php?id=<?= $s['id'] ?>" class="text-decoration-none">
            <div class="card bg-white shadow-sm border-0 d-flex flex-row gap-3 align-items-center p-2" style="border-radius: 4px; border: 1px solid #eee !important;">
              <div style="width: 100px; height: 70px; background-color: #eee; border-radius: 4px; overflow: hidden; display: flex; align-items: center; justify-content: center; position: relative;">
                <img src="uploads/<?= htmlspecialchars($s['thumb'] ?? '') ?>" alt="thumbnail" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.style.display='none'">
                <?php if($s['price'] > 0): ?>
