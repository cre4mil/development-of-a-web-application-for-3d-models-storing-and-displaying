...
const DEFAULT_MODEL_ORDER = 'm.created_at DESC';
$orderBySql = DEFAULT_MODEL_ORDER;
...
<div class="container mt-4">
    ...
    <form method="get" action="index.php" id="filterForm" class="filter-bar-custom mb-4">
      <div class="row g-3 align-items-end">
        ...
        <!-- Search Input -->
        <div class="col-md-3">
          <label class="form-label small text-muted fw-bold text-uppercase mb-1" for="filter_search"><i class="bi bi-search me-1"></i> ค้นหา</label>
          <div class="input-group">
            <input type="search" id="filter_search" name="search" class="form-control" placeholder="ค้นหาชื่อ, รายละเอียด หรือแท็ก..." value="<?= htmlspecialchars($search) ?>">
        ...
        <div class="col-md-2 col-6">
          <label class="form-label small text-muted text-uppercase mb-1 d-block" for="filter_tag"><i class="bi bi-tag me-1"></i> หมวดหมู่ (Tag)</label>
          <select id="filter_tag" name="tag" class="form-select form-select-sm" onchange="this.form.submit()">
        ...
        <!-- Date Dropdown -->
        <div class="col-md-2 col-6">
          <label class="form-label small text-muted text-uppercase mb-1 d-block" for="filter_date"><i class="bi bi-calendar-event me-1"></i> วันที่อัปโหลด</label>
          <select id="filter_date" name="date" class="form-select form-select-sm" onchange="this.form.submit()">
        ...
        <!-- Licenses Dropdown -->
        <div class="col-md-2 col-6">
          <label class="form-label small text-muted text-uppercase mb-1 d-block" for="filter_license"><i class="bi bi-shield-check me-1"></i> สัญญาอนุญาต</label>
          <select id="filter_license" name="license" class="form-select form-select-sm" onchange="this.form.submit()">
        ...
        <!-- Sort By Dropdown -->
        <div class="col-md-2 col-6">
          <label class="form-label small text-muted text-uppercase mb-1 d-block" for="filter_sort"><i class="bi bi-sort-down me-1"></i> จัดเรียงโดย</label>
          <select id="filter_sort" name="sort" class="form-select form-select-sm" onchange="this.form.submit()">
  ...
  <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-3 g-4" id="model-grid">
    <?php foreach ($models as $m):
      ...
      $sizeText = file_exists($p) ? humanFilesize(filesize($p)) : 'Unknown';
    ?>
    <div class="col">
      <div class="card model-card h-100 border-0 shadow-sm" style="border-radius:12px; overflow:hidden; background-color: #fff; transition: transform 0.2s ease, box-shadow 0.2s ease;">
        ...
        <a href="model.php?id=<?= $mid ?>" class="d-block" style="aspect-ratio: 4/3; background-color: #f8f9fa; overflow: hidden; cursor: pointer;">
          <img class="model-thumbnail" src="uploads/<?= htmlspecialchars($m['thumb'] ?? '') ?>" alt="<?= htmlspecialchars($m['title']) ?>" style="width: 100%; height: 100%; object-fit: cover; transition: transform 0.3s ease;">
        </a>
...
<div class="pagination-section text-center my-4">
  <nav aria-label="การแบ่งหน้า"><ul class="pagination justify-content-center" id="pagination-nav">
    <?php
      function buildQuery($extra=[]){$p=$_GET;unset($p['page']);$p=array_merge($p,$extra);return'index.php?'.http_build_query($p);}
      $prev=max(1,$page-1); $next=min($totalPages,$page+1);
    ?>
    <li class="page-item <?= $page<=1?'disabled':'' ?>"><button class="page-link" data-target="<?= buildQuery(['page'=>1]) ?>">&laquo;</button></li>
    <li class="page-item <?= $page<=1?'disabled':'' ?>"><button class="page-link" data-target="<?= buildQuery(['page'=>$prev]) ?>">&lsaquo;</button></li>
    <li class="page-item disabled"><span class="page-link">หน้า <?= $page ?> / <?= $totalPages ?></span></li>
    <li class="page-item <?= $page>=$totalPages?'disabled':'' ?>"><button class="page-link" data-target="<?= buildQuery(['page'=>$next]) ?>">&rsaquo;</button></li>
    <li class="page-item <?= $page>=$totalPages?'disabled':'' ?>"><button class="page-link" data-target="<?= buildQuery(['page'=>$totalPages]) ?>">&raquo;</button></li>
  </ul></nav>
</div>
...
<script src="https://cdn.jsdelivr.net/npm/three@0.130.0/examples/js/loaders/FBXLoader.js" integrity="sha384-6f75FYQfp4EtgxljHW3QpNgmdz9ASfe1+O00T21f8bGabqeuS8XfHjXfjjjieGfu" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/three@0.130.0/examples/js/loaders/STLLoader.js" integrity="sha384-QF8EmP6pyNE+i7WmcltzC4ddzFVKDxfn5WD5gXyKTSE4SCw0R25TI+q0LUlnf7tq" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/three@0.130.0/examples/js/loaders/PLYLoader.js" integrity="sha384-TRjDrMoP2Iw2zIithJ7Pm10f16V6yXxbUwTEYL5urkonr6Zr+xZ2WDOj2ONVpnSd" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/three@0.130.0/examples/js/loaders/ColladaLoader.js" integrity="sha384-Ayd5ILydqZMBb7pROfS1nXmnCTp54eGKjBstFEZVQk+GO3ReMcCTzR9Z+RJ1X1fR" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/three@0.130.0/examples/js/loaders/TDSLoader.js" integrity="sha384-4wpQ8AgXEeR0Ac4yCctD9EllVESYdcfZeCJ1khJD54VCdMMklOF9kiIEh+kE8uKz" crossorigin="anonymous"></script>
<script>
