...
require_once __DIR__ . '/../backend/repositories/OrderStore.php';
require_once __DIR__ . '/../backend/repositories/PdoOrderRepository.php';
...
$orderService = new OrderService(new PdoOrderRepository($pdo));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_order') {
        $modelIds = Commerce::normalizeModelIds((array) ($_POST['model_ids'] ?? []));

        if (empty($modelIds)) { echo json_encode(['error' => 'no_models']); exit; }

        // อัปโหลดสลิป
        $slipError = Commerce::validateSlip($_FILES['slip'] ?? null);
        if ($slipError !== null) { echo json_encode(['error' => $slipError]); exit; }
        ...
        $quote = $orderService->quote($uid, $modelIds, PLATFORM_FEE_PCT);
        if ($quote === null) {
        ...
        try {
            $createdOrder = $orderService->create($uid, $modelIds, $slipUrl, $paymentStatus, $adminNote, PLATFORM_FEE_PCT);
        ...
        if ($createdOrder === null) { @unlink($slipDir . $slipFile); echo json_encode(['error' => 'no_valid_items']); exit; }

        echo json_encode([
            'ok'        => 1,
            'order_ref' => $createdOrder['order_ref'],
    ...
    if ($action === 'update_status') {
        ...
        if (!in_array($newStatus, ['approved', 'rejected'], true)) { echo json_encode(['error' => 'bad_status']); exit; }

        try {
            $result = $orderService->process($uid, $orderId, $newStatus, $note);
        } catch (Throwable $e) {
            echo json_encode(['error' => 'db_error', 'detail' => $e->getMessage()]); exit;
        }
        if ($result === 'forbidden') { http_response_code(403); }
        echo $result === 'ok'
            ? json_encode(['ok' => 1, 'status' => $newStatus])
