        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style="color:var(--muted);font-size:.75rem"><?= date(DISPLAY_DATETIME_FORMAT, strtotime($s['created_at'])) ?></td>
            </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style="color:var(--muted);font-size:.8rem"><?= date(DISPLAY_DATETIME_FORMAT, strtotime($p['created_at'])) ?></td>
              <td style="color:var(--muted);font-size:.8rem">
                <?= $p['transferred_at'] ? date(DISPLAY_DATETIME_FORMAT, strtotime($p['transferred_at'])) : '-' ?>
              </td>
              <td>
    ...
    <div class="tab-content" id="tab-bank">
      <div style="padding:1.75rem;max-width:480px;">
        ...
        <form method="POST" action="creator_earnings.php">
          ...
          <label class="form-label-c" for="bank_account_no">เลขบัญชี</label>
          <input type="text" id="bank_account_no" name="bank_account_no" class="form-input-c"
                 placeholder="เช่น 1234567890"
                 value="<?= htmlspecialchars($wallet['bank_account_no'] ?? '') ?>">

          <label class="form-label-c" for="bank_account_name">ชื่อเจ้าของบัญชี</label>
          <input type="text" id="bank_account_name" name="bank_account_name" class="form-input-c"
