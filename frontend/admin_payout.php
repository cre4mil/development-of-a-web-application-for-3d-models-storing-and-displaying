...
require_once __DIR__ . '/../backend/services/Commerce.php';
require_once __DIR__ . '/../backend/services/Payout.php';
require_once __DIR__ . '/../backend/services/PayoutService.php';
require_once __DIR__ . '/../backend/repositories/PayoutStore.php';
require_once __DIR__ . '/../backend/repositories/PdoPayoutRepository.php';
...
$payoutService = new PayoutService(new PdoPayoutRepository($pdo));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_payout') {
        ...
        try {
            $payoutId = $payoutService->create($creatorId, $amount, $uid);
            echo $payoutId === null
                ? json_encode(['error' => 'insufficient_balance'])
                : json_encode(['ok' => 1, 'payout_id' => $payoutId]);
        } catch (Throwable) {
    ...
    if ($action === 'transfer_payout') {
        ...
        if (!empty($_FILES['slip']) && $_FILES['slip']['error'] === UPLOAD_ERR_OK) {
            $slipError = Commerce::validateSlip($_FILES['slip']);
            if ($slipError !== null) {
                echo json_encode(['error' => $slipError]);
                exit;
            }
            ...
            if (!move_uploaded_file($_FILES['slip']['tmp_name'], $dir . $fname)) {
                echo json_encode(['error' => 'slip_save_failed']);
                exit;
            }
            $slipPath = 'uploads/payout_slips/' . $fname;
        } else {
            echo json_encode(['error' => 'slip_required']);
            exit;
        }

        try {
            echo $payoutService->transfer($payoutId, $uid, $slipPath)
                ? json_encode(['ok' => 1])
                : json_encode(['error' => 'not_found_or_done']);
        } catch (Throwable) {
            echo json_encode(['error' => 'db_error']);
        }
        exit;
    ...
    if ($action === 'reject_payout') {
        ...
        try {
            echo $payoutService->reject($payoutId, $note)
                ? json_encode(['ok' => 1])
                : json_encode(['error' => 'not_found_or_done']);
        } catch (Throwable) {
            echo json_encode(['error' => 'db_error']);
    ...
    if ($action === 'update_platform_fee') {
        ...
        if (!Payout::isValidPlatformFee($newFee)) {
...
<div class="modal-overlay" id="payoutModal">
  <div class="modal-box">
    ...
    <div style="margin-bottom:1rem">
      <div class="form-label-c">Creator</div>
      <div id="payoutCreatorName" style="font-weight:600;color:var(--text)"></div>
    </div>
    <div style="margin-bottom:1rem">
      <div class="form-label-c">ยอดคงเหลือ</div>
...
<div class="modal-overlay" id="transferModal">
  <div class="modal-box">
    ...
    <div style="margin-bottom:1rem;">
      <div class="form-label-c">ยอดโอน</div>
      <div id="transferAmount" style="font-weight:800;font-size:1.2rem;color:var(--accent2)"></div>
    </div>
    <label class="form-label-c" for="transferSlipInput">สลิปการโอน</label>
    <div class="slip-drop" id="transferDropZone">
      <input type="file" id="transferSlipInput" accept="image/jpeg,image/png,image/webp" aria-label="สลิปการโอน">
...
async function submitPayout(){
  ...
  const res = await fetch('admin_payout.php', {
    ...
    body: new URLSearchParams({action:'create_payout', creator_id:creatorId, amount:amount.toFixed(2), csrf:<?= json_encode($_SESSION['csrf']) ?>})
  });
...
if(tsInput) {
    tsInput.addEventListener('change', function(){
      ...
      r.onload = function(e){ tsImg.src=e.target.result; tsPreview.style.display='block'; };
      r.readAsDataURL(f);
    });
}
...
async function rejectPayout(payoutId){
  ...
  const res = await fetch('admin_payout.php', {
    method:'POST',
    headers:{'Content-Type':'application/x-www-form-urlencoded'},
    body: new URLSearchParams({action:'reject_payout', payout_id:payoutId, note:note, csrf:<?= json_encode($_SESSION['csrf']) ?>})
  });
  const d = await res.json();
...
async function updatePlatformFee() {
  ...
  if (isNaN(val) || !(val >= 0) || val > 100) {
    ...
    alertEl.innerHTML = '\x3Ci class="bi bi-exclamation-triangle-fill me-1"\x3E\x3C/i\x3Eกรุณาระบุค่าระหว่าง 0–100 เท่านั้น';
    alertEl.classList.remove('d-none');
    return;
  }

  btn.disabled = true;
  btn.innerHTML = '\x3Cspan class="spinner-border spinner-border-sm"\x3E\x3C/span\x3E กำลังบันทึก...';

  const res = await fetch('admin_payout.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({ action: 'update_platform_fee', fee_pct: val, csrf: <?= json_encode($_SESSION['csrf']) ?> })
  });
  const d = await res.json();

  btn.disabled = false;
  btn.innerHTML = '\x3Ci class="bi bi-check-circle-fill"\x3E\x3C/i\x3E บันทึก';

  if (d.ok) {
    const creatorPct = (100 - d.fee_pct).toFixed(1);
    document.getElementById('currentFeeDisplay').innerHTML =
      d.fee_pct.toFixed(1) + '\x3Cspan style="font-size:1rem;font-weight:600;color:#2563eb;"\x3E%\x3C/span\x3E';

    alertEl.style.background = '#dcfce7';
    alertEl.style.color = '#15803d';
    alertEl.innerHTML = '\x3Ci class="bi bi-check-circle-fill me-1"\x3E\x3C/i\x3Eบันทึกสำเร็จ! ค่าธรรมเนียมใหม่: \x3Cstrong\x3E' + d.fee_pct.toFixed(1) + '%\x3C/strong\x3E · Creator ได้รับ \x3Cstrong\x3E' + creatorPct + '%\x3C/strong\x3E';
    alertEl.classList.remove('d-none');
    setTimeout(function(){ alertEl.classList.add('d-none'); }, 5000);
  } else {
    alertEl.style.background = '#fee2e2';
    alertEl.style.color = '#b91c1c';
    alertEl.innerHTML = '\x3Ci class="bi bi-exclamation-triangle-fill me-1"\x3E\x3C/i\x3Eเกิดข้อผิดพลาด: ' + (d.error || 'unknown');
    alertEl.classList.remove('d-none');
  }
}
</script>
