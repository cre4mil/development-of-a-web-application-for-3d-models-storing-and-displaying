...
document.addEventListener('click', async e => {
  ...
  const res = await fetch('payment.php', {
    ...
    body: new URLSearchParams({ action: 'update_status', order_id: btn.dataset.id, status: 'approved', note: '' })
  });
  const d = await res.json();
...
document.addEventListener('click', async e => {
  ...
  const res = await fetch('payment.php', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: new URLSearchParams({ action: 'update_status', order_id: btn.dataset.id, status: 'rejected', note: note })
  });
  const d = await res.json();
  if (d.ok) { loadOrders(currentFilter); }
  else { alert(d.error || 'เกิดข้อผิดพลาด'); btn.disabled = false; }
