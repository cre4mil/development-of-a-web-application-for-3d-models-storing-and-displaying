...
function maskPaymentAccount(string $id): string {
    ...
    $len = strlen($id);
    if ($len === 10) {
        // เบอร์โทร: 08x-xxx-xx78
        return substr($id, 0, 3) . '-xxx-' . substr($id, -2);
    } elseif ($len === 13) {
        // บัตรประชาชน: x-xxxx-xxxxx-xx-x
        return substr($id, 0, 1) . '-xxxx-xxxxx-' . substr($id, -3, 2) . '-' . substr($id, -1);
    } elseif ($len > 4) {
        // เลขบัญชีธนาคารทั่วไป
        return str_repeat('x', max(0, $len - 4)) . substr($id, -4);
    }
    return $id;
}
